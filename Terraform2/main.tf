...
# Retrieve subnet IDs individually
data "aws_subnet" "default_subnet1" {
  id = "subnet-0203ef7bc0c395695"  # Replace with actual subnet ID
}
...
output "subnet_ids" {
  value = [data.aws_subnet.default_subnet1.id, data.aws_subnet.default_subnet2.id]
}
